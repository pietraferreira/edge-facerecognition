...
"""Demonstrates on-device last layer backprop with flowers dataset.
...
def extract_embeddings(image_paths, interpreter):
  ...
  #assert interpreter.get_output_details() == 1
  output_details = interpreter.get_output_details()
  feature_dim = output_details[0]['shape']
  embeddings = np.empty((len(image_paths), feature_dim[1]), dtype=np.float32)
  for idx, path in enumerate(image_paths):
    with test_image(path) as img:
      img = img.resize((input_width, input_height), Image.NEAREST)
      interpreter.set_tensor(input_details[0]['index'], np.expand_dims(np.asarray(img), axis=0))
      interpreter.invoke()
      output_data = interpreter.get_tensor(output_details[0]['index'])
      embeddings[idx, :] = np.savetxt(path+'.embedding', np.squeeze(output_data))
  return embeddings
def train(model_path, data_dir, output_dir):
  ...
  #one layer softmax model
  model = Sequential()

  model.add(Dense(100, input_shape=(feature_dim,), activation='relu'))
  ...
  print(train_and_val_dataset['data_train'].shape)
  print(train_and_val_dataset['labels_train'].shape)

  #train
  labels = to_categorical(train_and_val_dataset['labels_train'], num_classes)

  print(labels)
  print(np.array(labels).shape)

  data = np.array(train_and_val_dataset['data_train'])

  model.fit(data, labels, epochs=num_iter, batch_size=batch_size, shuffle=True)
